...
#define GAME_FPS 60
#define FRAME_DELAY 1000 / GAME_FPS 
...
player_union player;
player_union player_cpy;
movement_union player_movement;

tile_union tile;
iso_tile_union iso_tile;
...
int init_game() { 
    ...
    init_dynamic_entity(&iso_tile.entity_comp, 0, 0, 64, 64, 0, 0);
    init_image2d(&iso_tile.image_comp, "Resources/tile_iso.png", 64, 64, 0, 0);
...
void game_loop() {
    ...
    player.scroll.y += (int) ((player.entity_comp.pos.y - player.scroll.y - ((SCREEN_H * 0.75) - (TILE_SIZE * 0.5))) / 10);
    ...
    switch (le_g_event.type) {
        ...
        case SDL_KEYDOWN:
            switch (le_g_event.key.keysym.scancode) {
                ...
                case SDL_SCANCODE_S:
                    player.scroll.x += (rand() % 16) - 4;
                    player.scroll.y += (rand() % 16) - 4;
                    break;
	...
	for (y = 0; y < (SCREEN_H / TILE_SIZE); y++) {
	    for (x = 0; x < (SCREEN_W / TILE_SIZE); x++) {
            ...
            ||  ((x == 16 || x == 15 || x == 14) && y == 12)
                ...
                tile.entity_comp.pos.x = (x * TILE_SIZE) - player.scroll.x;
                tile.entity_comp.pos.y = (y * TILE_SIZE) - player.scroll.y;

                if ((player.entity_comp.pos.x - (10 * TILE_SIZE) < tile.entity_comp.pos.x) && (tile.entity_comp.pos.x < player.entity_comp.pos.x + (10 * TILE_SIZE)))
	...
	for (y = 0; y < (SCREEN_H / TILE_SIZE); y++) {
	    for (x = 0; x < (SCREEN_W / TILE_SIZE); x++) {
            ...
            if ((x == 7 || x == 8) && (y == 0 || y == 1 || y == 2)) {
                iso_tile.entity_comp.pos.x = ((x * 34 - y * 34) + 64) - player.scroll.x;
                if (y == 1)
                    iso_tile.entity_comp.pos.y = ((x * 16 + (y + effect) + y * 16 + (y + effect)) + 64) - player.scroll.y;
                else 
                    iso_tile.entity_comp.pos.y = ((x * 16 + (y - effect) + y * 16 + (y - effect)) + 64) - player.scroll.y;
                draw_dynamic_entity_img(&iso_tile.entity_comp, &iso_tile.image_comp);
            }
        }
	}
	...
	draw_dynamic_entity_img(&player_cpy.entity_comp, &player_cpy.image_comp);
